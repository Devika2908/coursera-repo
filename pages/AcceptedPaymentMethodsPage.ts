...
export class AcceptedPaymentMethodsPage extends BasePage {
  ...
  async getPaymentMethods(): Promise<string[]> {
    await this.page.waitForSelector(locators.AcceptedPaymentMethodsPage.paymentList, {
      state: 'visible',
      timeout: 90000
    });

    const listLocator = this.page.locator(locators.AcceptedPaymentMethodsPage.paymentItems);
    const count = await listLocator.count();
    const texts: string[] = [];
